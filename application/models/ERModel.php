...
class ERModel extends CI_Model{
	...
	public function save($data = null) {
        if ($data != null) {
            if ($this->db->insert('et_reservas', $data)) {
                return true;
            }
        }
    }
    ...
    public function update($data = null) {
        if ($data != null) {
            ...
            if ($this->db->update('et_reservas', $data)) {
                return true;
            }
        }
    }
    ...
    public function truncate() {
        if ($this->db->truncate("et_reservas")) {
            return true;
        }
    }
    ...
    public function listing() {
        $this->db->select('*');
        $this->db->join('parciais', 'parciais.partialteam=pr_team', 'inner');
        $this->db->join('atletas', 'atletas.atletasid=pr_atleta', 'inner');
        $this->db->order_by("partialpoints", "desc");
        return $this->db->get("et_reservas")->result();
    }
	
	public function search($pr_id) {
        $this->db->where("pr_id", $pr_id);
        return $this->db->get("et_reservas")->row_array();
    }
	...
	public function getsquadout($pr_team) {
        $this->db->where("pr_team", $pr_team);
        $this->db->join('eliminatorio', 'eliminatorio.et_id=pr_team', 'inner');
        $this->db->join('atletas', 'atletas.atletasid=pr_atleta', 'inner');
