<?php
class PartialatletasModel extends CI_Model{
    protected $pa_id;
    protected $pa_team;
    protected $pa_atleta;
	...
	public function save($data = null) {
        if ($data != null) {
            if ($this->db->insert('partialatletas', $data)) {
                return true;
            }
        }
    }
    ...
    public function update($data = null) {
        if ($data != null) {
            ...
            if ($this->db->update('partialatletas', $data)) {
                return true;
            }
        }
    }
    ...
    public function delete($pa_id) {
        if ($pa_id != null) {
            ...
            if ($this->db->delete("partialatletas")) {
                return true;
            }
        }
    }
    ...
    public function truncate() {
        if ($this->db->truncate("partialatletas")) {
            return true;
        }
    }
    ...
    public function listing() {
        $this->db->select('*');
        $this->db->join('partial', 'partial.partialteam=pa_team', 'inner');
        $this->db->join('atletas', 'atletas.atletasid=pa_atleta', 'inner');
        $this->db->order_by("partialpoints", "desc");
        return $this->db->get("partialatletas")->result();
    }
	
	public function search($pa_id) {
        $this->db->where("pa_id", $pa_id);
        return $this->db->get("partialatletas")->row_array();
    }
